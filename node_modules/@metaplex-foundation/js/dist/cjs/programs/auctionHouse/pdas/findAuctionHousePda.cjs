'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var buffer = require('buffer');
var AuctionHouseProgram = require('../AuctionHouseProgram.cjs');
var Pda = require('../../../types/Pda.cjs');

const findAuctionHousePda = (creator, treasuryMint, programId = AuctionHouseProgram.AuctionHouseProgram.publicKey) => {
  return Pda.Pda.find(programId, [buffer.Buffer.from('auction_house', 'utf8'), creator.toBuffer(), treasuryMint.toBuffer()]);
};

exports.findAuctionHousePda = findAuctionHousePda;
//# sourceMappingURL=findAuctionHousePda.cjs.map
