...
const printNewEditionOperationHandler = {
  handle: async (operation, metaplex) => {
    ...
    const newAssociatedToken = findAssociatedTokenAccountPda.findAssociatedTokenAccountPda(newMint.publicKey, newOwner, tokenProgram, associatedTokenProgram);
    const sharedInput = {
      lamports,
      edition,
      newMint,
    ...
    if (operation.input.via === 'vault') {
      ...
      const originalTokenAccount = (_operation$input$orig2 = operation.input.originalTokenAccount) !== null && _operation$input$orig2 !== void 0 ? _operation$input$orig2 : findAssociatedTokenAccountPda.findAssociatedTokenAccountPda(originalMint, originalTokenAccountOwner.publicKey, tokenProgram, associatedTokenProgram);
      transactionBuilder = printNewEditionBuilder({
        via: 'token',
